...
ConVar SurvivorLimitMin;
ConVar SurvivorLimitMax;
ConVar InfectedLimitMin;
ConVar InfectedLimitMax;
...
public void OnPluginStart()
{
	...
	L4DGameMode      = FindConVar("mp_gamemode");
	SurvivorLimitMin = CreateConVar("l4d_survivor_min", "4", "Minimum amount of survivors (bots will be spawned to meet this number)", CVAR_FLAGS,true, 1.00, true, 31.00);
	SurvivorLimitMax = CreateConVar("l4d_survivor_max", "16", "Maximum amount of survivors in competitive gamemodes", CVAR_FLAGS,true, 1.00, true, 31.00);
	InfectedLimitMin = CreateConVar("l4d_infected_min", "4", "Minimum amount of infected (bots will be spawned to meet this number)", CVAR_FLAGS, true, 1.00, true, 31.00);
	InfectedLimitMax = CreateConVar("l4d_infected_max", "16", "Maximum amount of infected in competitive gamemodes", CVAR_FLAGS, true, 1.00, true, 31.00);

	// Remove limits for survivor/infected
	SetConVarBounds(L4DSurvivorLimit, ConVarBound_Upper, true, 31.0);
	SetConVarBounds(L4DInfectedLimit, ConVarBound_Upper, true, 31.0);	
	HookConVarChange(InfectedLimitMax, OnInfectedChanged);	HookConVarChange(L4DInfectedLimit, OnInfectedChanged);
	HookConVarChange(SurvivorLimitMax, OnSurvivorChanged);	HookConVarChange(L4DSurvivorLimit, OnSurvivorChanged);
	HookConVarChange(L4DGameMode, OnGameModeChanged);	
	...
	TankHpMulti    = CreateConVar("director_tank_hpmulti","0.25","Tanks HP Multiplier (multi*(survivors-4)). Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.00,false);
	SiHpMulti      = CreateConVar("director_si_hpmulti","0.00","SI HP Multiplier (multi*(survivors-4)). Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.00,false);
	CiSpMulti      = CreateConVar("director_ci_multi","0.25","Infected spawning rate Multiplier (multi*(survivors-4)). Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.00,false);
	...
	RegAdminCmd("sm_l4dtcheck", L4DTCheck, ADMFLAG_ROOT, "Check L4DT");
...
public void OnInfectedChanged (Handle c, const char[] o, const char[] n)
{
	L4DInfectedLimit.IntValue = InfectedLimitMax.IntValue;
}

public void OnSurvivorChanged (Handle c, const char[] o, const char[] n)
{
	L4DSurvivorLimit.IntValue = SurvivorLimitMax.IntValue;
}

public void OnGameModeChanged (Handle c, const char[] o, const char[] n)
{
	ConVar gm = view_as<ConVar>(c);
	gm.GetString(gameMode, sizeof(gameMode));
...
bool l4dt() 
{ 
	if (FindConVar("left4dhooks_version") != null && FindConVar("left4dhooks_version").FloatValue > 0.0) 
		return true; 
	else 
		return false;
}  // Is Left 4 Downtown 1/2 loaded?

public Action L4DTCheck(int client, int args) {
	ReplyToCommand(client, "L4DT: %i", l4dt());
...
void SpawnCheck()
{
	...
	int iSurvivorMin    = SurvivorLimitMin.IntValue;
	int iSurvivorMax    = iHumanSurvivor  >  iSurvivorMin ? iHumanSurvivor  : iSurvivorMin ;
	...
	if (iSurvivor < iSurvivorMin) PrintToConsoleAll("[SV] Spawning %d bot(s)", iSurvivorMin - iSurvivor);
	...
	for(; iSurvivor < iSurvivorMin; iSurvivor++)
	{
...
public Action Timer_AutoJoinTeam(Handle timer, int userid)
{
	...
	if (!client || !IsClientInGame(client) || IsFakeClient(client) || GetClientTeam(client) > TEAM_SPECTATOR || GetBotOfIdle(client)) return Plugin_Handled;
...
public Action Event_BotReplacedPlayer(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (IsFakeClient(player)) return Plugin_Continue; 		// if "player" is a bot, don't do anything (side effect of creating new bots)
...
public Action Join_Game(int client, int args)
{
	...
	else if (InfectedLimitMax.IntValue <= GetTeamPlayers(TEAM_INFECTED, false) && SurvivorLimitMax.IntValue <= GetTeamPlayers(TEAM_SURVIVOR, false))
	{
		PrintToChat(client, "Both teams are full.");
	}
	else if (InfectedLimitMax.IntValue <= GetTeamPlayers(TEAM_INFECTED, false)) FakeClientCommand(client,"sm_survivor");
	else if (SurvivorLimitMax.IntValue <= GetTeamPlayers(TEAM_SURVIVOR, false)) FakeClientCommand(client,"sm_infected");
...
{	
	...
	else if(InfectedLimitMax.IntValue <= GetTeamPlayers(TEAM_INFECTED, false))
	{
...
void DisplayTeamMenu(int client)
{
	...
	Format(title_survivor, sizeof(title_survivor), "Survivors (%d/%d) - %d Bot(s)", GetTeamPlayers(TEAM_SURVIVOR, false), SurvivorLimitMax.IntValue, CountAvailableBots(TEAM_SURVIVOR));
	...
	{
		if ( InfectedAllowed) Format(title_infected, sizeof(title_infected), "Infected (%d/%d) - %d Bot(s)", GetTeamPlayers(TEAM_INFECTED, false), InfectedLimitMax.IntValue, CountAvailableBots(TEAM_INFECTED));
	...
	{
		Format(title_infected, sizeof(title_infected), "Infected (%d/%d)", GetTeamPlayers(TEAM_INFECTED, false), InfectedLimitMax.IntValue);  // doesn't show how many bots are alive
	}
...
void AutoDifficultyCheck()
{
	...
	if (TankHpMulti.FloatValue > 0.0){   // if not 0
	...
	if(!InfectedAllowed && SiSpMore.FloatValue > 0.0 && l4dt() && !StrEqual(gameMode, "survival"))    // Not in survival, versus, scavenge or realism versus, l4dt required
	{
...
public Action BotInfectedCounter(Handle timer, bool recheck)
{
	...
	if (recheck && !AnySurvivorLeftSafeArea()) return Plugin_Continue;  // Disable counter if false start due to round restart
	
	if (!MoreSiBotsVersus.BoolValue || GetTeamPlayers(TEAM_INFECTED, false) >= InfectedLimitMin.IntValue) // if no extra bots wanted
	{
...
void SiSpawnCheck()
{
	...
	for (int i = GetTeamPlayers(TEAM_INFECTED, true) + CountSiQueue(); i < InfectedLimitMin.IntValue; i++)
	{
		AddSiToQueue();
	}
...
int GetOldestSi()
{
	...
	int   iOldest     =   			    -1;
	for (int i = 0; i < InfectedLimitMax.IntValue; i++)
	{
...
int PickSiType()
{
	...
	int iInfected = 0;
	int iSurvivors = GetSurvivorTeam();
	
	// We need to make sure we don't try to spawn more infected than we actually have slots for
	if (MaxClients - iSurvivors < InfectedLimitMax.IntValue)
		iInfected = MaxClients - iSurvivors;
	else
		iInfected = InfectedLimitMax.IntValue;
		
	//PrintToServer("MaxClients %d iSurvivors %d iInfected %d", MaxClients, iSurvivors, iInfected);
